...
class UserRepository extends BaseRepository
{
    ...
    public function create(array $data)
    {
        return DB::transaction(function () use ($data) {
            ...
            if ($user) {
                 ...
                 */
                $user->assign(config('access.users.default_role'));
            }
    ...
    public function update($id, array $input, $image = false)
    {
        ...
        if ($image) {
            ...
            $image_path = auth()->user()->avatar_location;  // Value is not URL but directory file path
            ...
            // Image Directory
            $imageDirectory = public_path() . '/' . config('core.avatar_path');
            
            // Paths
            $relativePath = config('core.avatar_path').'/'.$filename;
            ...
            // $source = $resize->store('/avatars', 'public');
            // $resize->save(public_path($path));
            // $image = \Image::make($source)->fit(120);
            $user->avatar_location = $relativePath;
        } else {
