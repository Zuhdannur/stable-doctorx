...
jQuery(function () {
    ...
    $(document).ready(function(){
        ...
        $(document).on('click', '.add', function() {
            ...
            html += '<td colspan="2">';
            html += '<select name="item_radeem['+(i+1)+']" id="'+(i+1)+'item_radeem" class="form-control item_radeem" width="70%">';
            html += '{!! $radeem !!}';
            html += '</select>';
            html += '</td>';
            html += '<td>';
    ...
    function calc(){
        calc_nominal_point();
        let total = parseInt(toNumber($('#remaining_payment').html()));
        let potongan_point = calc_potongan_point();
        $('#total_potongan_point').html(formatRupiah(potongan_point, '{{ setting()->get('currency_symbol') }}'));
        total = total - potongan_point
        if(total < 0) total = 0
    ...
    function calc_nominal_point(){
        ...
        $('#table_logic tbody .radeem').each(function(i, element){
            ...
            if (html != '') {
                ...
                // sum globat point
                $point_total = total_point + $point_total;
    ...
    function calc_kembalian() {
        let total = parseInt(toNumber($('#total').html()));
        ...
        totalKembalian = toNumber(total_receive) - total;
        ...
        Swal.fire({
            ...
            if (result.value) {
                try {
                    ...
                    if($point_total > const_point) throw new Error('Point Pasien Tidak Mencukupi !!!')
                } catch (error) {
                    $.alert({
                        title: "Kesalahan",
                        icon: 'fa fa-warning',
