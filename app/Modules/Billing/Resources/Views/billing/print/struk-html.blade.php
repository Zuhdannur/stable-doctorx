...
<div class="container">
    <div class="d-flex justify-content-center">
        <div class="col" style='margin-bottom:10px;margin-top:8px;text-align: center'>
            ...
            <p>{{ strtoupper(@$billing->klinik->alamat) }}</p>
        </div>
    </div>
    <div class="row">
        <div class="col" style='margin-bottom:10px;margin-top:8px'>
            ...
            <table class="custom-table">
                <tr>
                    <td><h5><span style='width:18mm;display: inline-block;'>Nama Pasien&nbsp;</span></h5></td>
                    <td>:</td>
                    <td><h5>{{ $billing->patient->patient_name }}</h5></td>
                </tr>
                <tr>
                    <td><h5><em class="wrapword"><span style='width:18mm;display: inline-block;'>No Invoice</span></em></h5></td>
                    <td>:</td>
                    <td><h5><em class="wrapword">{{ $billing->patient->patient_unique_id }}</em></h5></td>
                </tr>
                <tr>
                    <td><p><em class="wrapword"><span
                                    style='width:18mm;display: inline-block;'>Alamat&nbsp;</span></em></p></td>
                    <td>:</td>
                    <td>{{ isset($billing->patient->address) ? ucwords(strtolower($billing->patient->address)) : '-' }}
                        , {{ isset($billing->patient->city->name) ? ucwords(strtolower($billing->patient->city->name)) : '-' }} {{ $billing->patient->zip_code }}</td>
                </tr>
            </table>


{{--            <h5><em class="wrapword"><span--}}
{{--                        style='width:18mm;display: inline-block;'>ID&nbsp;</span>:&nbsp;{{ $billing->patient->patient_unique_id }}--}}
{{--                </em></h5>--}}
{{--            <p><em class="wrapword"><span--}}
{{--                        style='width:18mm;display: inline-block;'>Alamat&nbsp;</span>:&nbsp;{{ isset($billing->patient->address) ? ucwords(strtolower($billing->patient->address)) : '-' }}--}}
...
{{--                </em></p>--}}
    ...
    <div class="row" style='margin-bottom:10px;'>
        <div class="col">
            ...
            <h5><span style='width:18mm;display: inline-block;'>Dibuat Pada&nbsp;</span>: {{ $billing->created_at }}
            </h5>
            ...
            <h5><span
                    style='width:18mm;display: inline-block;'>Metode Pembayaran&nbsp;</span>: {{ @\App\Modules\Billing\Models\PaymentHistory::where('invoice_id',$billing->id)->first()->account->account_name }}
            </h5>
                ...
                @foreach($billing->invDetail as $key => $item)
                    ...
                    @if ($item->payment_step == $billing->is_payment)
                        <tr>
                            ...
                            <td><strong>{{ currency()->rupiah($amount , setting()->get('currency_symbol')) }}</strong>
                            </td>
                        </tr>
                        <tr>
        ...
        <div class="col">
            <table style='border-collapse: collapse;'>
                ...
                <tr>
                    ...
                    <td class='wrapword' style="font-size:9pt">
                        <strong>{{ currency()->rupiah($billing->subtotal, setting()->get('currency_symbol')) }}</strong>
                    </td>
                </tr>
                {{--                        <tr>--}}
                ...
                @if ($billing->patient->membership)
                    <tr>
                        ...
                        <td class='wrapword' style="font-size:9pt">
                            <strong>{{ currency()->rupiah($billing->radeem_point, setting()->get('currency_symbol')) }}</strong>
                        </td>
                    </tr>
                @endif
                @if(!empty($billing->tax_total))
                    <tr>
                        <th style='width:35mm;text-align: right;padding-right: 10px;'>Total Tax</th>
                        <td class='wrapword' style="font-size:9pt">
                            <strong>{{ currency()->rupiah($billing->tax_total, setting()->get('currency_symbol')) }}</strong>
                        </td>
                    </tr>
                @endif
                @if(!empty($billing->discountprice))
                    <tr>
                        <th style='width:35mm;text-align: right;padding-right: 10px;'>
                            Diskon {{ intval($billing->discount_percent).'%' }}</th>
                        <td class='wrapword' style="font-size:9pt">
                            <strong>{{ currency()->rupiah($billing->discountprice, setting()->get('currency_symbol')) }}</strong>
                        </td>
                    </tr>
                @endif
                <tr>
                    ...
                    <td class='wrapword' style="font-size:9pt">
                        <strong>{{ currency()->rupiah($billing->total_ammount -  $billing->discountprice, setting()->get('currency_symbol')) }}</strong>
                    </td>
                </tr>
                <tr>
                    <th style='width:35mm;text-align: right;padding-right: 10px;'>Jumlah yang telah dibayarkan</th>
                    <td class='wrapword' style="font-size:9pt">
                        <strong>{{ currency()->rupiah($billing->total_pay -  $billing->discountprice, setting()->get('currency_symbol')) }}</strong>
                    </td>
                </tr>
                @if(!empty($billing->remaining_payment))
                    <tr>
                        ...
                        <td class='wrapword' style="font-size:9pt">
                            <strong>{{ currency()->rupiah($billing->remaining_payment, setting()->get('currency_symbol')) }}</strong>
                        </td>
                    </tr>
                @endif
                <tr>
                    ...
                    <td class='wrapword' style="font-size:9pt">
                        <strong>{{ currency()->rupiah($billing->last_payment->total_pay - $billing->discountprice, setting()->get('currency_symbol')) }}</strong>
                    </td>
                </tr>
                <tr>
                    <th style='width:35mm;text-align: right;padding-right: 10px;'>Diterima</th>
                    <td class='wrapword' style="font-size:9pt">
                        <strong>{{ currency()->rupiah($billing->last_payment->in_paid, setting()->get('currency_symbol')) }}</strong>
                    </td>
                </tr>
                <tr>
                    <th style='width:35mm;text-align: right;padding-right: 10px;'>Kembalian</th>
                    <td class='wrapword' style="font-size:9pt">
                        <strong>{{ currency()->rupiah(($billing->last_payment->in_paid - $billing->last_payment->total_pay) + $billing->discountprice, setting()->get('currency_symbol')) }}</strong>
                    </td>
                </tr>
                </tbody>
            </table>
    ...
    @if($billing->patient->membership)
        <div class="row">
            <div class="col">
                <hr style='width:66mm;border-bottom : 1.5px dashed #2a2929;border-top: none;margin-bottom:3px;'>
                <p class="wrapword">Point Membership Anda Saat ini adalah
                    : {{ $billing->patient->membership->total_point }}</p>
                <hr style='width:66mm;border-bottom : 1.5px dashed #2a2929;border-top: none;margin-top:3px;'>
            </div>
        </div>
    @endif
    ...
    <div class="row">
        <div class="col">
            <hr style='width:66mm;border-bottom : 1.5px dashed #2a2929;border-top: none;margin-bottom:3px;'>
            <p class="wrapword">Facebook : {{ setting()->get('copyright') }}</p>
            <p class="wrapword"> Instagram : {{ setting()->get('copyright_link') }}</p>
            <hr style='width:66mm;border-bottom : 1.5px dashed #2a2929;border-top: none;margin-top:3px;'>
        </div>
    </div>
    <div class="row">
