...
@section('title', app_name(). ' | '. __('accounting::menus.kas.send'))
        ...
        <div class="block-header block-header-default">
            <h3 class="block-title">
                ...
                <small class="text-muted">
                    &nbsp;
                    @lang('accounting::labels.cash.send')
                </small>
            </h3>
                ...
                <div class="row-mt-4">
                    <div class="col">
                        <div class="form-group row">
                            <label for="acc_cash" class="form-control-label">Kirim dari : {{ $account->account_name." ($account->account_code)" }}</label>
                ...
                <div class="row-mt-4">
                    <div class="row clearfix">
                        ...
                        <table class="table table-bordered table-hover tabel-striped tabel-vcenter" id="tab_logic">
                            <thead>
                                <th></th>
                                <th style='width:35%'>Pembayaran Untuk</th>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    <td>
                                        <select name="acc_purchase[1]" id="1acc_purchaseselect2" class="form-control acc_purchase required" data-placeholder="Pilih" style="width: 100%">
                                            {!! $accountList !!}
                                        </select>
                                    </td>
                                    <td>
        ...
        jQuery(function(){
            ...
            var validator = jQuery('.js-validation-bootstrap').validate({
                ...
                submitHandler: function(form){
                    ...
                    $.ajax({
                        type: 'POST',
                        url: "{{ route('admin.accounting.cash.store_send') }}",
                        data: data,
                        processData: false,
                        contentType: false,
                        dataType: 'json',
            ...
            $(document).ready(function(){
                ...
                $(document).on('click', '.add', function() {
                    ...
                    html += '<td>';
                    html += '<select name="acc_purchase['+(i+1)+']" id="'+(i+1)+'acc_purchaseselect2" class="form-control acc_purchase required" data-placeholder="Pilih" style="width: 100%">';
                    html +=  '{!! $accountList !!}';
                    html +=  '</select>';
                    html += '</td>';
                    html += '<td>';
            ...
            function updateIds() {
                $('#tab_logic tbody tr').each(function(i, element) {
                    var html = $(this).html();
                    if (html != '') {
                        $(this).find('.acc_purchase').attr('name', 'acc_purchase['+(i+1)+']');
