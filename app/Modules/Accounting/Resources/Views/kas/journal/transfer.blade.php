    ...
    <div class="block" id="my-block2">
        ...
        <div class="block-content">
            ...
            <div class="row px-5 pt-5">
                <div class="col-3 contact-details">
                    @php
                        ...
                        $acc_trf = $trx->journal->load('account')->where('tags', 'is_trf')->first();
                    @endphp
                    
                   <h5 class="block-title">Transfer Dari : {{ isset($cash->account) ? $cash->account->account_code .' - '. $cash->account->account_name : ''}}</h5>
                   <h5 class="block-title">Transfer Ke : {{ isset($acc_trf->account) ? $acc_trf->account->account_code .' - '. $acc_trf->account->account_name : ''}}</h5>
                   <h5 class="block-title">Nominal : {{ isset($cash) ? currency()->rupiah($cash->value, setting()->get('currency_symbol')) : ''}}</h5>
                ...
                <div class="invoice-details col-3 offset-3 text-right">
                    <span><em>Dibuat Pada : {{ $trx->created_at }}</em></span><br>
            ...
            <div class="table-responsive push">
                <h3 class="block-title text-center">Jurnal Transaksi</h3>
