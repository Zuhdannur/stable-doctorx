...
@section('title', app_name(). ' | '. __('accounting::menus.kas.receipt'))
        ...
        <div class="block-header block-header-default">
            <h3 class="block-title">
                ...
                <small class="text-muted">
                    &nbsp;
                    @lang('accounting::labels.cash.receipt')
                </small>
            </h3>
            ...
            <form method="post" class="js-validation-bootstrap" id="receipt_form" autocomplete="off" enctype="multipart/form-data">
                <div class="row-mt-4">
                    <div class="col">
                        <div class="form-group row">
                            <label for="acc_cash_id" class="col-md-12 form-control-label">Setor Ke : {{ $account->account_name." ($account->account_code)" }}</label>
                ...
                <div class="row-mt-4">
                    <div class="col">
                        <div class="form-group row">
                            <div class="col-4">
                                <label for="buyer" class="form-control-label">Pembayar</label>
                    ...
                    <div class="row clearfix">
                        <div class="col-md-12">
                            ...
                            <table class="table table-bordered table-hover table-striped table-vcenter" id="tab_logic">
                                <thead>
                                    <th></th>
                                    <th style='width:35%'>Terima Dari</th>
                                ...
                                <tbody>
                                    <tr>
                                        ...
                                        <td>
                                            <select name="acc_receipt[1]" id="1acc_receiptselect2" class="form-control acc_receipt required" data-placeholder="Pilih" style="width: 100%">
                                                {!! $cashAccountList !!}
                                            </select>
                                        </td>
                                        <td>
                                        ...
                                        <td>
                                            <input type="text" name='total[1]' placeholder='0' class="form-control tanpa-rupiah text-right total required"/>
                                        </td>
                                    </tr>
                                </tbody>
                                <tfoot>
        ...
        jQuery(function () {
            ...
            $(document).ready(function() {
                ...
                $(document).on('click', '.add', function() {
                    ...
                    html += '<td>'+
                                '<select name="acc_receipt['+(i+1)+']" id="'+(i+1)+'acc_receiptselect2" class="form-control acc_receipt required" data-placeholder="Pilih" style="width: 100%">'+
                                        '{!! $cashAccountList !!}'+
                                '</select>'+
                            '</td>';
                    html += '<td>'+
                                '<textarea name="acc_desc['+(i+1)+']" id="'+(i+1)+'acc_desc" rows="1" class="form-control acc_desc"></textarea>'+
                            '</td>';
                    html += '<td>'+
                                '<select name="tax_form['+(i+1)+']" id="'+(i+1)+'tax_formselect2" class="form-control tax_form" data-placeholder="Pilih" style="width: 100%">'+
                                   '{!! $taxList !!}'+
                                '</select>'+
                            '</td>';
                    html += '<td>'+
                                '<input type="text" name="total['+(i+1)+']" id="'+(i+1)+'total" placeholder="0" class="form-control tanpa_rupiah text-right total required" />'+
                            '</td>';
                ...
                $("#tab_logic").on('change', '.tax_form',function () {
                    ...
                    var tax_total = parseInt($('#tax_total').val());

                    if(isNaN(tax)){
                        tax = 0;
                    }else{
                        if(!isNaN(tax_total)){
                            tax = tax_total + parseInt(tax);
                        }
                    }
            ...
            var validator = jQuery('.js-validation-bootstrap').validate({
                ...
                submitHandler: function(form) {
                    ...
                    $.ajax({
                        type: "POST",
                        url:"{{ route('admin.accounting.cash.store_receipt') }}",
                        data: data,
                        processData: false,
                        contentType: false,
                        dataType: 'json',
            ...
            function updateIds() {
                $('#tab_logic tbody tr').each(function(i, element) {
                    var html = $(this).html();
                    if (html != '') {
                        $(this).find('.acc_receipt').attr('name', 'acc_receipt['+(i+1)+']');
            ...
            function calc_total(){
                ...
                tax = $('#tax_total').val() / 100;

                tax_sum = parseInt((total * tax));
                total_amount = total + tax_sum;
