...
@section('title', app_name().' | '.__('accounting::menus.reports.neraca'))
                    ...
                    <div class="col-md-12">
                        <table id="tableStudent" class="js-table-sections table table-sm table-striped table-bordered table-hover js-table-sections-enabled table-vcenter" width="100%">
                            <thead class="thead-light">
                                <tr>
                                    <th colspan="3" class="text-center">Neraca Keuangan</th>
                                </tr>
                            </thead>
                            @php
                                $type = 'init';
                            ...
                            @foreach ($neraca as $key => $item)
                                ...
                                @if ($type != $item['type'])
                                    <tr>
                                        <td colspan="3" class="bg-primary text-light">{{ config('finance_account.type_list.'.$item['type']) }}</td>
                                    </tr>
                                @endif
                                ...
                                @php
                                    ...
                                    if($item['type'] == 2 || $item['type'] == 3){
                                        $sum_neraca = $sum_neraca + $item['value'];
                                    }
                                    // $type_total = $type_total + $item['value'];
                            ...
                            @endforeach
                                ...
                                <tr>
                                    <td colspan="2" class="text-right">Pendapatan Bersih Periode Ini</td>
                                    <td class="text-right">{{ currency()->rupiah($sumPendapatan, setting()->get('currency_symbol')) }}</td>
                                </tr>
                                <tr class="bg-secondary text-white">
                                    <td colspan="2" style="padding-left:60px" class='text-center'>Total {{ $type_name }}</td>
                                    <td class="text-right">{{ currency()->rupiah($type_total, setting()->get('currency_symbol')) }}</td>
                                ...
                                <tr class="bg-secondary text-white">
                                    <td colspan="2" style="padding-left:60px" class='text-center'>Total Kewajiban dan Ekuitas</td>
                                    <td class="text-right">{{ currency()->rupiah($sum_neraca + $sumPendapatan, setting()->get('currency_symbol')) }}</td>
                                </tr>
                        </table>
                    </div>
                </div>
