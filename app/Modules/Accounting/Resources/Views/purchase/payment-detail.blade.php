...
<h2 class="content-heading d-print-none">
    <a href="{{ route('admin.accounting.purchase') }}" class="btn btn-sm btn-rounded btn-success float-right">Daftar Pembelian</a>
    ...
    <div class="block-content">
        ...
        <div class="row px-5 pt-5">
            <div class="col-3 contact-details">
                @switch($trx->purchasePayment->purchase->status)
                    @case(1)
                        <h2 class='text-success'>Lunas</h2>
                        @break
                    @default
                        @php
                            $due_date = date_create(date('Y-m-d' ,strtotime($trx->purchasePayment->purchase->due_date)));
            ...
            <div class="invoice-details col-3 offset-3 text-right">
                ...
                <h6>Jatuh Tempo: {{ strtotime($trx->purchasePayment->purchase->due_date) > 0 ? date('d/m/y', strtotime($trx->purchasePayment->purchase->due_date)) : ' - ' }}</h6>
            </div>
        </div>
        <!-- END Biaya Info -->
        ...
        <div class="table-responsive push">
            <table class="table table-bordered table-hover">
                ...
                <tbody>
                    @php
                        $purchase = isset($trx->purchasePayment->purchase->financeTrx->transaction_code) ? $trx->purchasePayment->purchase->financeTrx->transaction_code : '';
                        $tmp_pembayaran = $trx->journal->where('tags', 'is_cash')->first();
                        $pembayaran = isset($tmp_pembayaran->value) ? $tmp_pembayaran->value : 0;
                        $total = isset($trx->purchasePayment->purchase->total) ? $trx->purchasePayment->purchase->total : 0;
                        $sisa_tagihan = isset($trx->purchasePayment->purchase->remain_payment) ? $trx->purchasePayment->purchase->remain_payment : 0;
                    @endphp
                    <tr>                                    
                        <td>
                            <a href="{{ route('admin.accounting.account.journal.detail', $trx->purchasePayment->purchase->financeTrx )}}">
                                <p class="font-w600 mb-5">{{ $purchase }}</p>
                            </a>
                        </td>
